    ...
    tabItems(
      tabItem(tabName = "explore", h2("Examin the relation between diamonds characteristics and price"),
              ...
              fluidRow(
                ...
                box(width=3,
                    selectInput('color','Choose coloring variable', choices=subset(col_names, !(col_names %in% c('price', 'carat'))), selected='clarity'))
              )
      ),
...
server <- function(input, output) {
  ...
  output$plot1 <- renderPlot({
    ggplot(df(), aes_string(x = 'carat', y = 'price', col=input$color))+ geom_smooth()+ geom_point()+ scale_y_continuous("Price (USD)", limits= c(0, 2000))+ scale_x_continuous("Carat", limits= c(0, 1))
  })
